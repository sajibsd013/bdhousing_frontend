  ...
  <div class="card material-table" ref="table">
    ...
    <div v-if="paginate" class="table-footer">
      ...
      <div :class="{ 'datatable-info': true }">
        <span
          >{{
            (tableData["currentPage"] - 1) * currentPerPage
              ? (tableData["currentPage"] - 1) * currentPerPage
              : 1
          }}
          -{{
            Math.min(
              ...
              currentPerPage * tableData["currentPage"]
            )
          }}
        </span>
        <span>
...
export default {
  ...
  methods: {
    nextPage() {
      if (
        ...
        this.currentPerPage * this.tableData["currentPage"]
      )
        ++this.tableData["currentPage"];
    },

    previousPage() {
      if (this.tableData["currentPage"] > 1) --this.tableData["currentPage"];
    },
  ...
  watch: {
    ...
    rows(newRows, oldRows) {
      ...
      if (newRows !== oldRows) this.tableData["currentPage"] = 1;
    },
  },
    ...
    paginated() {
      ...
      if (this.paginate && this.currentPerPage !== -1)
        paginatedRows = paginatedRows.slice(
          (this.tableData["currentPage"] - 1) * this.currentPerPage,
          this.currentPerPage === -1
            ? paginatedRows.length + 1
            : this.tableData["currentPage"] * this.currentPerPage
        );
